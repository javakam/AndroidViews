 ...
 */
public class GroupItemDecoration<Sticky,Item, Expand> extends RecyclerView.ItemDecoration {
    ...
    protected Map<Integer, Item> mGroup = new HashMap<>();
    ...
    protected void onDrawOverGroup(Canvas c, RecyclerView parent) {
        ...
        Item group = getCroup(firstVisiblePosition);
        if (group == null) {
            return;
        }
        ...
        Item nextGroup = getCroup(firstVisiblePosition + 1);
     ...
     */
    protected Item getCroup(int position) {
        while (position >= 0) {
     ...
     */
    public void notifyDataSetChanged(BaseRecyclerAdapter<Sticky,Item, Expand,BaseViewHolder> adapter) {
        mGroup.clear();
        if (adapter == null) {
            return;
        }
        ...
        for (int i = 0; i < adapter.getGroupCount(); i++) {
            if (i == 0) {
                mGroup.put(isHasHeader ? 1 : 0, (Item) adapter.getGroup(i));
                key += adapter.getChildCount(i) + (isHasHeader ? 1 : 0);
            } else {
                mGroup.put(key, (Item) adapter.getGroup(i));
