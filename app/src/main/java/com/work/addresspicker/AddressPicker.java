...
public class AddressPicker extends FrameLayout {
    ...
    private AddressDao mAddressDao;                             //地址数据DAO
    ...
    public AddressPicker(Context context, AddressDao addressDao, @StyleRes int resId) {
    ...
    private void initView() {
        ...
        List<AddressBean> mAddressList = mAddressDao.getAllProvince();
        mAdapter.init(mAttributes.mPickPlatform == MODE_PAD ?
                R.layout.item_address_pad : R.layout.item_address_phone, mAddressList);
        mAdapter.setOnAddressPickedListener(new OnAddressPickedListener() {
            @Override
            public void onPick(View v, final AddressBean bean, int position) {
                //地区级别 0:全国 1：省、直辖市 2：市、区 3：县、区 4 ：乡、镇 5：村、居委会
                newTab(bean, Integer.valueOf(bean.ilevel) - 1, position);
            }
        });
        mRecycler.setAdapter(mAdapter);
        mTvHint.setText(HintEnum.PROVINCE.getHintDesc());
        TabLayout.Tab firstTab = mTabLayout.newTab();
        firstTab.setTag(new TabTagParam(0, -1, mAddressList));
    ...
    private void newTab(final AddressBean bean, int oldIndex, int position) {
        ...
        List<AddressBean> newTabData = mAddressDao.getAddressBySSUPERID(bean.sid);
        if (newTabData.size() > 0) {
            TabLayout.Tab newTab = mTabLayout.getTabAt(newIndex);
            TabTagParam newTabParam = new TabTagParam(newIndex, -1, newTabData);
            if (newTab == null) {
    ...
    private class AddressPickerAdapter
        ...
        public void init(@LayoutRes int layoutRes, List<AddressBean> data) {
        ...
        public void setItems(List<AddressBean> data) {
            this.data = data;
        }
        ...
        public void onBindViewHolder(@NonNull AddressPickerAdapter.AddressSelectHolder holder,
            ...
            holder.tvArea.setText(bean.sname);
    ...
    public interface OnAddressPickedListener {
         ...
         * @param position
         */
        void onPick(View v, AddressBean bean, int position);
    }
    ...
    private static class TabTagParam implements Serializable {
        ...
        List<AddressBean> cacheList;

        TabTagParam(int tabIndex, int pickedIndex, List<AddressBean> cacheList) {
